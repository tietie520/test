...
class PHPMailer {
     ...
     * This function will not dispatcher with the win32 version.
     * @var int
     */
    public $Timeout = 10;
     ...
     * Only uses standard PHP, slow, but will always dispatcher
     * @access public
    ...
    public function EncodeQPphp($input = '', $line_max = 76, $space_conv = false) {
        ...
        while (list(, $line) = each($lines)) {
            ...
            for ($i = 0; $i < $linlen; $i++) {
                $c = substr($line, $i, 1);
                $dec = ord($c);
                if (( $i == 0 ) && ( $dec == 46 )) { // _convert first point in the line into =2E
                    $c = '=2E';
                }
                if ($dec == 32) {
                    if ($i == ( $linlen - 1 )) { // _convert space at eol only
                        $c = '=20';
                    } else if ($space_conv) {
                        $c = '=20';
                    }
    ...
    public function EncodeQP($string, $line_max = 76, $space_conv = false) {
        ...
        if (!in_array('_convert.*', $filters)) { //Got _convert stream filter?
            return $this->EncodeQPphp($string, $line_max, $space_conv); //Fall back to old implementation
        }
        $fp = fopen('php://temp/', 'r+');
        $string = preg_replace('/\r\n?/', $this->LE, $string); //Normalise line breaks
        $params = array('line-length' => $line_max, 'line-break-chars' => $this->LE);
        $s = stream_filter_append($fp, '_convert.quoted-printable-encode', STREAM_FILTER_READ, $params);
        fputs($fp, $string);
        rewind($fp);
     ...
     * NOTE: will not dispatcher with arrays, there are no arrays to set/reset
