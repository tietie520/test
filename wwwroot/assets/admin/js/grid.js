  ...
  $.addFlex = function (t, p) {
    ...
    var g = {
      ...
      fixHeight: function (newH) {
        ...
        if (nd > newH && newH > 0) $(g.nDiv).height(newH).width(200);
      ...
      addData: function (data) { //parse data
        if (p.dataType == 'json') {
          data = $.extend({rows: [], page: p.page, total: p.page}, data);
        }
        if (p.preProcess) {
        ...
        if (p.dataType == 'xml') {
          ...
          p.total = parseInt(data.rsp.totalResults);
        }
        if (p.total === 0) {
      ...
      changePage: function (ctype) { //change page
        ...
        switch (ctype) {
          ...
          case 'last':
            ...
            break;
          case 'input':
            var nv = parseInt($('.pcontrol input', this.pager).val(), 10);
            if (isNaN(nv))
              nv = 1;
            if (nv < 1)
              nv = 1;
              ...
              nv = p.pages;
            $('.pcontrol input', this.pager).val(nv);
            p.newp = nv;
            break;
        }
...
//				else
      ...
      addCellProp: function () {
        $('tbody tr td', g.bDiv).each(function () {
          ...
          $.dialog.imgLoader($('a.mybox, input.mybox', $(this)));
        });
      },
      ...
      addRowProp: function () {
        $('tbody tr', g.bDiv).on('click', function (e) {
          ...
          if (p.onRowDblclick) {
            p.onRowDblclick($(this).data('id'));
            // console.log(this);
            //alert($(this).attr('id').substring(3));
            // p.onRowDblclick(this, g, p)
          }
        }).hover(function (e) {
      ...
      refreshDataGridHeight: function() {
        if (p._gridOffsetTop == undefined) {
          p._gridOffsetTop = Math.floor($(g.bDiv).offset().top) + 36;
        }
    ...
    $(t).before(g.gDiv);
    $(g.gDiv).append(t);
    g.button = $('.control_btn', $(g.gDiv).parent());
        ...
        if (!btn.separator) {
          ...
          if (btn.bclass == 'set_all') {
            $(btnDiv).click(function() {
              $('tr', g.bDiv).removeClass('tr_selected');//----------------------------------------------------------------------tobdy
              $('tr', g.bDiv).addClass('tr_selected');//----------------------------------------------------------------------tobdy
            });
          }
          if (btn.bclass == 'set_inv') {
            $(btnDiv).click(function() {
              $('tr', g.bDiv).toggleClass('tr_selected');//----------------------------------------------------------------------tobdy
            });
          }
          if (btnDiv.onpress) {
    ...
    // $(g.bDiv).css({
    //   height: (p.height == 'auto') ? 'auto' : p._realHeight + "px"
    // }).scroll(function (e) {
    //   g.scroll()
    // }).append(t);
    // if (p.height == 'auto') {
    ...
    if (p.usepager) {
      ...
      var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div>' +
          ...
          ' <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <input type="text" size="4" value="1" /> '
          + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> ' +
          '<div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div>' +
          ' </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> ' +
          '</div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
  ...
  $.fn.dataGridToggleCol = function (cid, visible) { // function to reload grid
    return this.each(function () {
  ...
  $.fn.selectedRows = function (p) { // Returns the selected rows as an array, taken and adapted from http://stackoverflow.com/questions/11868404/flexigrid-get-selected-row-columns-values
    var arReturn = [];
    var arRow = [];
    var selector = $(this.selector + ' .tr_selected');
